<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

class CreateCertificacaoTreinamentoTable extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        Schema::create('certificacao_treinamento', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('certificacao_id')->onDelete('cascade');
            $table->unsignedBigInteger('treinamento_id')->onDelete('cascade');
            $table->timestamps();

            $table->foreign('certificacao_id')->references('id')->on('certificacoes');
            $table->foreign('treinamento_id')->references('id')->on('treinamentos');
        });
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::dropIfExists('certificacao_treinamento');
    }
}
